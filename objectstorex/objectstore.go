...
func New(cfg Config) (Client, error) {
	registerObjectStoreMetrics(cfg)
	started := time.Now()
	objectStoreLogger(cfg).Info("objectstorex opening", logx.Bool("metrics_enabled", cfg.MetricsEnabled), logx.String("endpoint", cfg.Endpoint), logx.String("bucket", cfg.Bucket))
	if err := cfg.Validate(); err != nil {
		return nil, observeObjectStoreInit(cfg, started, err)
	}
	open, ok := drivers[cfg.Driver]
	if !ok {
		return nil, observeObjectStoreInit(cfg, started, ErrUnknownDriver)
	}
	client, err := open(cfg)
	if err != nil {
		return nil, observeObjectStoreInit(cfg, started, err)
	}
	_ = observeObjectStoreInit(cfg, started, nil)
	return observeObjectStore(client, cfg), nil
}
