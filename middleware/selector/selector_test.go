...
func TestMatch(t *testing.T) {
	tests := []struct {
		...
		{
			name: "/example/kernel",
			ctx:  transport.NewServerContext(context.Background(), &Transport{operation: "/example/kernel"}),
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			next = Server(testMiddleware).Prefix("/hello/").Regex(`/test/[0-9]+`).
				Path("/example/kernel").Build()(next)
...
func TestMatchClient(t *testing.T) {
	tests := []struct {
		...
		{
			name: "/example/kernel",
			ctx:  transport.NewClientContext(context.Background(), &Transport{operation: "/example/kernel"}),
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
...
func TestFunc(t *testing.T) {
	tests := []struct {
		...
		{
			name: "/kernel.aisphere.io/kernel",
			ctx:  transport.NewServerContext(context.Background(), &Transport{operation: "/kernel.aisphere.io/kernel"}),
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			next = Server(testMiddleware).Match(func(_ context.Context, operation string) bool {
				if strings.HasPrefix(operation, "/kernel.aisphere.io") || strings.HasSuffix(operation, "world") {
					return true
				}
				return false
			}).Build()(next)
	...
	tests := []struct {
		...
		},
		{
			...
			ctx: transport.NewServerContext(context.Background(), &Transport{
				...
				headers:   &mockHeader{map[string][]string{"X-Test": {"test2"}, "aisphereio": {"kernel"}}},
			}),
		},
		{
			name: "/test.Name/1234",
		...
		},
		{
			name: "/kernel.aisphere.io/kernel",
			ctx: transport.NewServerContext(context.Background(), &Transport{
				operation: "/kernel.aisphere.io/kernel",
				headers:   &mockHeader{map[string][]string{"X-Test": {"test"}}},
			}),
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			next = Server(testMiddleware).Match(func(ctx context.Context, _ string) bool {
				...
				if tr.RequestHeader().Get("aisphereio") == "kernel" {
					return true
				}
				return false
			}).Build()(next)
