...
## âš ï¸ Prime directive (read first)
...
## ðŸš« Hard rules (violations will fail CI)
...
// âœ?REQUIRED
return errorx.NotFound("AIHUB_SKILL_NOT_FOUND", "æŠ€èƒ½ä¸å­˜åœ¨",
    errorx.WithMetadata("skill_id", id),
)
return errorx.Wrap(err, "AIHUB_SKILL_CREATE_FAILED",
    errorx.WithMessage("åˆ›å»ºæŠ€èƒ½å¤±è´?),
)
```
...
## ðŸ“š Examples â€?find by scenario BEFORE writing code
...
func (h *SkillHandler) Create(ctx context.Context, req *CreateSkillRequest) (*Skill, error) {
    if req.Name == "" {
        return nil, errorx.BadRequest("AIHUB_SKILL_NAME_REQUIRED", "æŠ€èƒ½åç§°ä¸èƒ½ä¸ºç©?)
    }
    if err := h.rt.Access.Require(ctx, access.Check{
        Resource: "aihub:skill:*",
        Action:   "skill.create",
    }); err != nil {
        return nil, errorx.Forbidden("AIHUB_SKILL_CREATE_DENIED", "æ²¡æœ‰åˆ›å»ºæƒé™",
...
func (r *SkillRepo) Find(ctx context.Context, id string) (*Skill, error) {
    ...
    if errors.Is(err, gorm.ErrRecordNotFound) {
        return nil, errorx.NotFound("AIHUB_SKILL_NOT_FOUND", "æŠ€èƒ½ä¸å­˜åœ¨",
            errorx.WithCause(err),
            errorx.WithMetadata("skill_id", id),
    ...
    if err != nil {
        return nil, errorx.Wrap(err, "AIHUB_SKILL_QUERY_FAILED",
            errorx.WithMessage("æŸ¥è¯¢æŠ€èƒ½å¤±è´?),
