...
func TestDecodeValues(t *testing.T) {
	form, err := url.ParseQuery("a=19&age=18&b=true&bool=false&byte=MTIz&bytes=MTIz&count=3&d=22.22&double=12.33&duration=" +
		...
		"strings=5566&string=aisphereio&timestamp=1970-01-01T00%3A00%3A20.000000002Z&uint32=32&uint64=64&very_simple.component=5566")
	if err != nil {
		t.Fatal(err)
	}
...
func TestPopulateMapField(t *testing.T) {
	query, err := url.ParseQuery("map%5Bkernel%5D=https://kernel.aisphere.io/")
	if err != nil {
		t.Fatal(err)
	}
	comp := &complex.Complex{}
	...
	err = populateMapField(field, comp.ProtoReflect().Mutable(field).Map(), []string{"map[kernel]"}, query["map[kernel]"])
	if err != nil {
		t.Fatal(err)
	}
	// Get the comp map field value
	if query["map[kernel]"][0] != comp.Map["kernel"] {
		t.Errorf("want: %s, got: %s", query["map[kernel]"], comp.Map["kernel"])
	}
}

func TestPopulateMapSepField(t *testing.T) {
	query, err := url.ParseQuery("map.name=kernel")
	if err != nil {
		t.Fatal(err)
	}
	comp := &complex.Complex{}
...
func TestParseURLQueryMapKey(t *testing.T) {
	tests := []struct {
		...
		{
			fieldName: "map[kernel]", field: "map", fieldKey: "kernel", err: nil,
		},
		{
		...
		{
			fieldName: "map[kernel]=", field: "", fieldKey: "", err: errInvalidFormatMapKey,
		},
		{
			fieldName: "[kernel]", field: "", fieldKey: "", err: errInvalidFormatMapKey,
		},
		{
		...
		{
			fieldName: "map.kernel.v2", field: "", fieldKey: "", err: errInvalidFormatMapKey,
		},
	}
	for _, test := range tests {
