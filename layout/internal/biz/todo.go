...
var (
	...
	ErrTodoNotFound = errorx.NotFound(errorx.Code(v1.ErrorReason_TODO_NOT_FOUND.String()), "todo not found")
	// ErrTodoInvalidArgument is returned when a todo request is invalid.
	ErrTodoInvalidArgument = errorx.BadRequest(errorx.Code(v1.ErrorReason_TODO_INVALID_ARGUMENT.String()), "invalid todo argument")
)
