...
func New(cfg Config) (Cache, error) {
	...
	started := time.Now()
	cacheLogger(cfg).Info("cachex opening", logx.Bool("metrics_enabled", cfg.MetricsEnabled), logx.Bool("cluster", cfg.Cluster), logx.Bool("sentinel", cfg.MasterName != ""))
	if err := cfg.Validate(); err != nil {
		return nil, observeCacheInit(cfg, started, err)
	}
	open, ok := drivers[cfg.Driver]
	if !ok {
		return nil, observeCacheInit(cfg, started, ErrUnknownDriver)
	}
	cache, err := open(cfg)
	if err != nil {
		return nil, observeCacheInit(cfg, started, err)
	}
	_ = observeCacheInit(cfg, started, nil)
	return observeCache(cache, cfg), nil
}
