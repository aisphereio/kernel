...
This module implements a `registry.Registrar` and `registry.Discovery` interface in kernel based `bilibili/discovery`.

[![go.dev reference](https://img.shields.io/badge/go.dev-reference-007d9c?logo=go&logoColor=white&style=flat-square)](https://pkg.go.dev/github.com/aisphereio/kernel/contrib/registry/discovery/v3)
...
func main() {
	...
	app := kernel.New(
		...
		kernel.Metadata(map[string]string{"color": "gray"}),
		// use Registrar
		kernel.Registrar(r),
	)
