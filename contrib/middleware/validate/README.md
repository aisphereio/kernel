# Validator Middleware for Aisphere Kernel Project

This module provides a middleware for Aisphere Kernel to validate request parameters, using schema defined in `.proto` files.

There used to be a middleware named `Validator` in Aisphere Kernel, which calls the generated validation functions
...
protovalidate no longer requires code generation at build time, but for compatibility with existing Aisphere Kernel
