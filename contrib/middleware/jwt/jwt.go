...
var (
	...
	ErrMissingKeyFunc         = errorx.Unauthorized(reason, "keyFunc is missing")
	ErrTokenInvalid           = errorx.Unauthorized(reason, "Token is invalid")
	ErrTokenExpired           = errorx.Unauthorized(reason, "JWT token has expired")
	ErrTokenParseFail         = errorx.Unauthorized(reason, "Fail to parse JWT token ")
	ErrUnSupportSigningMethod = errorx.Unauthorized(reason, "Wrong signing method")
	ErrWrongContext           = errorx.Unauthorized(reason, "Wrong context for middleware")
	ErrNeedTokenProvider      = errorx.Unauthorized(reason, "Token provider is missing")
	ErrSignToken              = errorx.Unauthorized(reason, "Can not sign token.Is the key correct?")
	ErrGetKey                 = errorx.Unauthorized(reason, "Can not get key while signing token")
)
