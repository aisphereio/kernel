...
type options struct {
	// counter: <client/server>_requests_code_total{kind, operation, code, error_code}
...
func Server(opts ...Option) middleware.Middleware {
	...
	return func(handler middleware.Handler) middleware.Handler {
		return func(ctx context.Context, req any) (any, error) {
			...
			var (
				code      = errorx.HTTPStatusOK
				errorCode = errorx.CodeOK.String()
				kind      string
				operation string
			)
			...
			if op.requests != nil {
				op.requests.Add(
					...
					metric.WithAttributes(
						...
						attribute.String(metricLabelErrorCode, errorCode),
					),
				)
			}
			if op.seconds != nil {
