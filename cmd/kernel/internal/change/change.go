...
// CmdChange is kernel change log tool
var CmdChange = &cobra.Command{
	Use:   "changelog",
	Short: "Get a kernel change log",
	Long:  "Get a kernel release or commits info. Example: kernel changelog dev or kernel changelog {version}",
	Run:   run,
}
...
func init() {
	if repoURL = os.Getenv("KRATOS_REPO"); repoURL == "" {
		repoURL = "https://github.com/aisphereio/kernel.git"
	}
