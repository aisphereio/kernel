...
func kernelHome() string {
	...
	home := filepath.Join(dir, ".kernel")
	if _, err := os.Stat(home); os.IsNotExist(err) {
		if err := os.MkdirAll(home, 0o700); err != nil {
			log.Fatalf("Failed to create kernel home %q: %v", home, err)
		}
	}
	return home
}

func kernelHomeWithDir(dir string) string {
	home := filepath.Join(kernelHome(), dir)
	if _, err := os.Stat(home); os.IsNotExist(err) {
		if err := os.MkdirAll(home, 0o700); err != nil {
			log.Fatalf("Failed to create kernel home directory %q: %v", home, err)
		}
	}
	return home
}
