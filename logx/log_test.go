...
func TestWith(t *testing.T) {
	...
	logger := With("service", "kernel", slog.Int("version", 2))
	logger.Info("hello", "kind", "test")

	if len(handler.records) != 1 {
		t.Fatalf("records len = %d, want 1", len(handler.records))
	}
	if got := handler.attrs[0]["service"]; got != "kernel" {
		t.Fatalf("service = %v, want %q", got, "kernel")
	}
...
func assertLogSource(t *testing.T, out *bytes.Buffer) {
	...
	if !strings.HasSuffix(file, "logx/log_test.go") {
		t.Fatalf("source.file = %q, want logx/log_test.go suffix", file)
	}
}
