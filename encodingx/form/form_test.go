...
import (
	...
	bdtest "github.com/aisphereio/kernel/internal/testdata/binding"
	"github.com/aisphereio/kernel/internal/testdata/complex"
	ectest "github.com/aisphereio/kernel/internal/testdata/encoding"
)

// This variable can be replaced with -ldflags like below:
// go test "-ldflags=-X github.com/aisphereio/kernel/encodingx/form.tagNameTest=form"
...
func TestFormCodecMarshal(t *testing.T) {
	...
	if !reflect.DeepEqual([]byte("password=kernel_pwd&username=kernel"), content) {
		t.Errorf("expect %s, got %s", "password=kernel_pwd&username=kernel", content)
	}
	...
	if !reflect.DeepEqual([]byte("username=kernel"), content) {
		t.Errorf("expect %s, got %s", "username=kernel", content)
	}
	...
	if !reflect.DeepEqual([]byte("id=1&name=kernel"), content) {
		t.Errorf("expect %s, got %s", "id=1&name=kernel", content)
	}
}
...
func TestFormCodecUnmarshal(t *testing.T) {
	...
	if !reflect.DeepEqual("kernel", bindReq.Username) {
		t.Errorf("expect %v, got %v", "kernel", bindReq.Username)
	}
	if !reflect.DeepEqual("kernel_pwd", bindReq.Password) {
		t.Errorf("expect %v, got %v", "kernel_pwd", bindReq.Password)
	}
}
...
func TestProtoEncodeDecode(t *testing.T) {
	in := &complex.Complex{
		...
		D:       22.22,
		Byte:    []byte("123"),
		Map:     map[string]string{"kernel": "https://kernel.aisphere.io/", "kernel_start": "https://kernel.aisphere.io/en/docs/getting-started/start/"},
		...
		String_:   &wrapperspb.StringValue{Value: "aisphereio"},
	...
	if "a=19&age=18&b=true&bool=false&byte=MTIz&bytes=MTIz&count=3&d=22.22&double=12.33&duration="+
		...
		"map%5Bkernel%5D=https%3A%2F%2Fkernel.aisphere.io%2F&map%5Bkernel_start%5D=https%3A%2F%2Fkernel.aisphere.io%2Fen%2Fdocs%2Fgetting-started%2Fstart%2F&"+
		"numberOne=2233&price=11.23&sex=woman&string=aisphereio&strings=3344&strings=5566"+
...
func TestDecodeStructPb(t *testing.T) {
	...
	query := `data={"name":"kernel"}&data_list={"name1": "kernel"}&data_list={"name2": "aisphereio"}`
	if err := encoding.GetCodec(Name).Unmarshal([]byte(query), req); err != nil {
		t.Fatal(err)
	}
	if "kernel" != req.Data.GetFields()["name"].GetStringValue() {
		t.Errorf("except %v, got %v", "kernel", req.Data.GetFields()["name"].GetStringValue())
	}
	if len(req.DataList) != 2 {
		t.Fatalf("except %v, got %v", 2, len(req.DataList))
	}
	if "kernel" != req.DataList[0].GetFields()["name1"].GetStringValue() {
		t.Errorf("except %v, got %v", "kernel", req.Data.GetFields()["name1"].GetStringValue())
	}
	if "aisphereio" != req.DataList[1].GetFields()["name2"].GetStringValue() {
		t.Errorf("except %v, got %v", "aisphereio", req.Data.GetFields()["name2"].GetStringValue())
	}
}
