# 从 Aisphere Kernel v2 迁移到 v3

Aisphere Kernel v3 清理了核心框架中的历史耦合，并将部分隐式依赖改为显式选择。本文总结了 [aisphereio/kernel#3820](https://github.com/aisphereio/kernel/issues/3820) 中已经落地的主要升级点。

## 1. 更新模块路径

将 Aisphere Kernel 导入路径从 `github.com/aisphereio/kernel/v2` 更新为 `github.com/aisphereio/kernel`。
...
contrib 模块同样使用 `/v3` 导入路径，例如 `github.com/aisphereio/kernel/contrib/middleware/jwt/v3`。

## 2. 显式选择 JSON Codec

v2 中 `encoding/json` 同时处理普通 Go JSON 值和 `proto.Message`，其中 protobuf 消息会使用 protobuf JSON 语义。v3 将核心 JSON codec 拆分：

- `github.com/aisphereio/kernel/encoding/json` 注册标准库 JSON codec，名称为 `json`。
- `github.com/aisphereio/kernel/encoding/protojson` 注册 protobuf JSON codec，名称为 `protojson`。
- `github.com/aisphereio/kernel/contrib/encoding/json/v3` 保留 v2 兼容的 `json` codec 行为，用于迁移期兼容。
...
Aisphere Kernel v3 使用标准库 `log/slog` API。依赖 `log.Logger`、`log.Helper`、`log.Valuer`、`log.NewStdLogger` 或 trace/service 辅助字段的代码，需要迁移到 `*slog.Logger`、`log.NewHandler` 和 `log.NewLogger`。
...
Aisphere Kernel 应用和 middleware 选项现在接收 `*slog.Logger`。
...
默认熔断器不再依赖 `github.com/aisphereio/aegis`。使用默认行为的代码无需调整：
...
完成依赖和导入更新后，重新生成 Aisphere Kernel 相关代码：
...
- [ ] 将旧 Aisphere Kernel 日志辅助 API 替换为基于 `log/slog` 的 API。
