...
func (n *Node) Pick() selector.DoneFunc {
	...
	return func(_ context.Context, di selector.DoneInfo) {
		...
		if di.Err != nil {
			...
			if stderrors.Is(di.Err, context.DeadlineExceeded) || stderrors.Is(di.Err, context.Canceled) ||
				errorx.IsUnavailable(di.Err) || errorx.IsTimeout(di.Err) || stderrors.As(di.Err, &netErr) {
				success = 0
			}
		}
