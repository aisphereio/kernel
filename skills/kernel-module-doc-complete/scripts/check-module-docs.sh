...
# Count public APIs
CONSTRUCTORS=$(grep -hE '^func [A-Z][a-zA-Z]+\(' $MODULE/*.go 2>/dev/null \
  | grep -v _test.go | grep -v '//' | wc -l || echo 0)
OPTIONS=$(grep -hE '^func With[A-Z][a-zA-Z]+\(' $MODULE/*.go 2>/dev/null \
  | grep -v _test.go | wc -l || echo 0)
INSPECT=$(grep -hE '^func [A-Z][a-zA-Z]+Of\(|^func Is[A-Z][a-zA-Z]+\(' $MODULE/*.go 2>/dev/null \
  | grep -v _test.go | wc -l || echo 0)
