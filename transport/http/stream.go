...
func (s *serverStream) Close(err error) error {
	...
	case streamModeSSE:
		...
		_ = s.sendSSE("error", ErrorResponseFromError(err))
		return nil
	case streamModeWebSocket:
		if s.conn == nil {
			return err
...
func (s *sseClientStream) RecvMsg(m any) error {
	for {
		...
		case "error":
			...
			if err := unmarshalStreamMessage(data, resp, s.decoder); err == nil && resp.Code != "" {
				return errorx.New(errorx.Code(resp.Code), errorx.WithMessage(resp.Message), errorx.WithPublicMetadataMap(resp.Metadata), errorx.WithRequestID(resp.RequestID), errorx.WithTraceID(resp.TraceID))
			}
...
func (client *Client) WebSocket(ctx context.Context, path string, opts ...CallOption) (ClientStream, error) {
	...
	h := func(ctx context.Context, _ any) (any, error) {
		...
		if client.r != nil {
			...
			if selectErr != nil {
				return nil, errorx.Unavailable("NODE_NOT_FOUND", "node not found", errorx.WithCause(selectErr), errorx.WithMetadata("select_error", selectErr.Error()))
			}
			done = doneFunc
			if client.insecure {
				scheme = "ws"
