...
import (
	...
	"github.com/aisphereio/kernel/internal/testdata/binding"
)
...
func TestBuildPath(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			request:      &binding.HelloRequest{Name: "kernel"},
			want:         "/helloworld/kernel",
		},
		{
			name:         "query",
			pathTemplate: "/helloworld/{name}",
			request:      &binding.HelloRequest{Name: "kernel", Sub: &binding.Sub{Name: "go"}},
			opts:         []BuildPathOption{WithQueryParams()},
			want:         "/helloworld/kernel?sub.naming=go",
		},
		{
			name:         "resource name",
			pathTemplate: "/v1/{name=publishers/*/books/*}",
			request:      &binding.HelloRequest{Name: "publishers/go/books/kernel"},
			opts:         []BuildPathOption{WithQueryParams()},
			want:         "/v1/publishers/go/books/kernel",
		},
		{
...
func TestBuildPathCompatibility(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			request:      &binding.HelloRequest{Name: "go", Sub: &binding.Sub{Name: "kernel"}},
			opts:         []BuildPathOption{WithQueryParams()},
			want:         "http://helloworld.Greeter/helloworld/go/sub?sub.naming=kernel",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/{name=publishers/*/books/*}/sub",
			request:      &binding.HelloRequest{Name: "publishers/go/books/kernel", Sub: &binding.Sub{Name: "kernel"}},
			opts:         []BuildPathOption{WithQueryParams()},
			want:         "http://helloworld.Greeter/helloworld/publishers/go/books/kernel/sub?sub.naming=kernel",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/{name=**}/sub",
			request:      &binding.HelloRequest{Name: "publishers/go/books/kernel", Sub: &binding.Sub{Name: "kernel"}},
		...
		},
		{
			...
			request:      &binding.HelloRequest{Sub: &binding.Sub{Name: "publishers/kernel"}},
			want:         "http://helloworld.Greeter/helloworld/publishers/kernel",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/sub/{sub.naming}",
			request:      &binding.HelloRequest{Sub: &binding.Sub{Name: "kernel"}, UpdateMask: &fieldmaskpb.FieldMask{Paths: []string{"name", "sub.naming"}}},
			want:         "http://helloworld.Greeter/helloworld/sub/kernel?updateMask=name,sub.naming",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/sub/[{sub.naming}]",
			request:      &binding.HelloRequest{Sub: &binding.Sub{Name: "kernel"}},
			want:         "http://helloworld.Greeter/helloworld/sub/[kernel]",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/[{name}]/sub/[{sub.naming}]",
			request:      &binding.HelloRequest{Name: "test", Sub: &binding.Sub{Name: "kernel"}},
			want:         "http://helloworld.Greeter/helloworld/[test]/sub/[kernel]",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/[{}]/sub/[{sub.naming}]",
			request:      &binding.HelloRequest{Sub: &binding.Sub{Name: "kernel"}},
		...
		},
		{
			...
			request:      &binding.HelloRequest{Name: "test", Sub: &binding.Sub{Name: "kernel"}},
			want:         "http://helloworld.Greeter/helloworld/{[name]}/[kernel]",
		},
		{
			pathTemplate: "http://helloworld.Greeter/helloworld/{}/[]/[{sub.naming}]",
			request:      &binding.HelloRequest{Sub: &binding.Sub{Name: "kernel"}},
			want:         "http://helloworld.Greeter/helloworld/{}/[]/[kernel]",
		},
	}
