...
func TestDefaultRequestEncoderUnknownCodec(t *testing.T) {
	...
	if se.Code() != errorx.Code("REQUEST_CONTENT_TYPE_UNSUPPORTED") {
		t.Errorf("expected %v, got %v", "REQUEST_CONTENT_TYPE_UNSUPPORTED", se.Code())
	}
}
...
func TestDefaultErrorDecoder(t *testing.T) {
	...
	resp2 := &http.Response{
		Header:     make(http.Header),
		StatusCode: 500,
		Body:       io.NopCloser(bytes.NewBufferString(`{"code":"FOO", "message":"hi"}`)),
	}
	err := DefaultErrorDecoder(context.TODO(), resp2)
	if err == nil {
		t.Errorf("expected error, got nil")
	}
	if errorx.HTTPStatusOf(err) != http.StatusInternalServerError {
		t.Errorf("expected %v, got %v", http.StatusInternalServerError, errorx.HTTPStatusOf(err))
	}
	if errorx.MessageOf(err) != "hi" {
		t.Errorf("expected %v, got %v", "hi", errorx.MessageOf(err))
	}
	if errorx.CodeOf(err) != errorx.Code("FOO") {
		t.Errorf("expected %v, got %v", "FOO", errorx.CodeOf(err))
	}
}
...
func TestNewClient(t *testing.T) {
	...
	_, err = NewClient(context.Background(), WithEndpoint("127.0.0.1:9999"), WithTLSConfig(&tls.Config{ServerName: "www.kernel.com", RootCAs: nil}))
	if err != nil {
		t.Error(err)
	}
	_, err = NewClient(context.Background(), WithDiscovery(&mockDiscovery{}), WithEndpoint("discovery:///aisphereio"))
	if err != nil {
		t.Error(err)
	}
	...
	_, err = NewClient(context.Background(), WithDiscovery(&mockDiscovery{}), WithEndpoint("https://kernel.aisphere.io/"))
	if err == nil {
	...
	client, err := NewClient(
		...
		WithEndpoint("discovery:///aisphereio"),
	...
	err = client.Invoke(context.Background(), http.MethodPost, "/go", map[string]string{"name": "kernel"}, nil, EmptyCallOption{}, &mockCallOption{})
	if err == nil {
		t.Error("err should not be equal to nil")
	}
	err = client.Invoke(context.Background(), http.MethodPost, "/go", map[string]string{"name": "kernel"}, nil, EmptyCallOption{}, &mockCallOption{needErr: true})
	if err == nil {
	...
	err = client.Invoke(context.Background(), http.MethodPost, "/go", map[string]string{"name": "kernel"}, nil, EmptyCallOption{})
	if err == nil {
...
func TestNewClientWithTLSDoesNotModifyDefaultTransport(t *testing.T) {
	...
	_, err := NewClient(context.Background(), WithEndpoint("127.0.0.1:9999"), WithTLSConfig(&tls.Config{ServerName: "www.kernel.com"}))
	if err != nil {
		t.Error(err)
	}
