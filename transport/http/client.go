...
import (
	...
	"github.com/aisphereio/kernel/selector/wrr"
...
func (client *Client) do(req *http.Request) (*http.Response, error) {
	...
	if client.r != nil {
		...
		if node, done, err = client.selector.Select(req.Context(), selector.WithNodeFilter(client.opts.nodeFilters...)); err != nil {
			return nil, errorx.Unavailable("NODE_NOT_FOUND", "node not found", errorx.WithCause(err), errorx.WithMetadata("select_error", err.Error()))
		}
		if client.insecure {
...
func DefaultRequestEncoder(_ context.Context, contentType string, in any) ([]byte, error) {
	...
	if codec == nil {
		return nil, errorx.BadRequest("REQUEST_CONTENT_TYPE_UNSUPPORTED", "unsupported request content type", errorx.WithMetadata("content_type", contentType))
	}
...
func DefaultErrorDecoder(_ context.Context, res *http.Response) error {
	...
	if err == nil {
		e := new(ErrorResponse)
		if err = CodecForResponse(res).Unmarshal(data, e); err == nil && e.Code != "" {
			return errorx.New(
				errorx.Code(e.Code),
				errorx.WithMessage(e.Message),
				errorx.WithHTTPStatus(res.StatusCode),
				errorx.WithPublicMetadataMap(e.Metadata),
				errorx.WithRequestID(e.RequestID),
				errorx.WithTraceID(e.TraceID),
			)
		}
	}
	return errorx.New(
