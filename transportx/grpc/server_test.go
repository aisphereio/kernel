...
func (s *server) SayHelloStream(streamServer pb.Greeter_SayHelloStreamServer) error {
	...
	for {
		...
		if in.Name == "error" {
			return errorx.BadRequest("custom_error", fmt.Sprintf("invalid argument %s", in.Name))
		}
		if in.Name == "panic" {
			panic("server panic")
		}
...
func (s *server) SayHello(_ context.Context, in *pb.HelloRequest) (*pb.HelloReply, error) {
	if in.Name == "error" {
		return nil, errorx.BadRequest("custom_error", fmt.Sprintf("invalid argument %s", in.Name))
	}
	if in.Name == "panic" {
		panic("server panic")
	}
...
func testClient(t *testing.T, srv *Server) {
	...
	reply, err := client.SayHello(context.Background(), &pb.HelloRequest{Name: "kernel"})
	t.Log(err)
	if err != nil {
		t.Errorf("failed to call: %v", err)
	}
	if !reflect.DeepEqual(reply.Message, "Hello kernel") {
		t.Errorf("expect %s, got %s", "Hello kernel", reply.Message)
	}
