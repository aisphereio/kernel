...
import (
	...
	transportpkg "github.com/aisphereio/kernel/transportx"
)
...
func TestServerSentEventStream(t *testing.T) {
	...
	if err := stream.Send(&binding.HelloRequest{Name: "kernel"}); err != nil {
		t.Fatal(err)
	}
	...
	if out.GetName() != "kernel" {
		t.Fatalf("expected %v, got %v", "kernel", out.GetName())
	}
...
func TestWebSocketStreamBindsPathQueryAndExchangesMessages(t *testing.T) {
	...
	srv.Route("/").GET("/ws/{name}", func(ctx Context) error {
		...
		if in.GetName() != "kernel" {
			return stream.Close(fmt.Errorf("expected path name kernel, got %s", in.GetName()))
		}
	...
	stream, err := client.WebSocket(context.Background(), "/ws/kernel?sub.naming=go", Accept("application/protojson"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestWebSocketStreamBindsNamedBodyField(t *testing.T) {
	...
	stream, err := client.WebSocket(context.Background(), "/ws/kernel", Accept("application/protojson"))
	if err != nil {
		t.Fatal(err)
	}
