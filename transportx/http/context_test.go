...
func TestContextHeader(t *testing.T) {
	w := wrapper{
		router: testRouter,
		req:    &http.Request{Header: map[string][]string{"name": {"kernel"}}},
		res:    nil,
		w:      responseWriter{},
	}
	h := w.Header()
	if !reflect.DeepEqual(h, http.Header{"name": {"kernel"}}) {
		t.Errorf("expected %v, got %v", http.Header{"name": {"kernel"}}, h)
	}
}
...
func TestContextForm(t *testing.T) {
	w := wrapper{
		router: testRouter,
		req:    &http.Request{Header: map[string][]string{"name": {"kernel"}}, Method: http.MethodPost},
		res:    nil,
		w:      responseWriter{},
	}
	form := w.Form()
	...
	w = wrapper{
		router: testRouter,
		req:    &http.Request{Form: map[string][]string{"name": {"kernel"}}},
		res:    nil,
		w:      responseWriter{},
	}
	form = w.Form()
	if !reflect.DeepEqual(form, url.Values{"name": {"kernel"}}) {
		t.Errorf("expected %v, got %v", url.Values{"name": {"kernel"}}, form)
	}
}
...
func TestContextQuery(t *testing.T) {
	w := wrapper{
		router: testRouter,
		req:    &http.Request{URL: &url.URL{Scheme: "https", Host: "github.com", Path: "aisphereio/kernel", RawQuery: "page=1"}, Method: http.MethodPost},
		res:    nil,
		w:      responseWriter{},
	}
	q := w.Query()
...
func TestContextBindQuery(t *testing.T) {
	w := wrapper{
		router: testRouter,
		req:    &http.Request{URL: &url.URL{Scheme: "https", Host: "aisphereio-dev", RawQuery: "page=2"}},
		res:    nil,
		w:      responseWriter{},
	}
...
func TestContextBindForm(t *testing.T) {
	w := wrapper{
		router: testRouter,
		req:    &http.Request{URL: &url.URL{Scheme: "https", Host: "aisphereio-dev"}, Form: map[string][]string{"page": {"2"}}},
		res:    nil,
		w:      responseWriter{},
	}
