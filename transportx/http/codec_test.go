...
import (
	...
	_ "github.com/aisphereio/kernel/encodingx/protojson"
...
func TestDefaultRequestDecoderProtoJSONRejectsScalarField(t *testing.T) {
	r, _ := http.NewRequest(http.MethodPost, "", io.NopCloser(bytes.NewBufferString(`"kernel"`)))
	r.Header.Set("Content-Type", "application/protojson")
...
func TestDefaultResponseEncoderProtoJSONRejectsScalarField(t *testing.T) {
	...
	err := DefaultResponseEncoder(w, r, "kernel")
	if err == nil {
...
func TestDefaultResponseDecoderProtoJSONRejectsScalarField(t *testing.T) {
	resp := &http.Response{
		Header:     http.Header{"Content-Type": []string{"application/protojson"}},
		StatusCode: http.StatusOK,
		Body:       io.NopCloser(bytes.NewBufferString(`"kernel"`)),
	}
...
func (errorCodec) Marshal(any) ([]byte, error) {
	return nil, stderrors.New("marshal error")
}
...
func TestDefaultErrorEncoder(t *testing.T) {
	var (
		...
		r, _ = http.NewRequest(http.MethodPost, "", nil)
		err  = errorx.New("NETWORK_AUTHENTICATION_REQUIRED", errorx.WithHTTPStatus(511), errorx.WithMessage("network authentication required"))
	)
	r.Header.Set("Content-Type", "application/json")
...
func TestDefaultErrorEncoderMarshalError(t *testing.T) {
	...
	DefaultErrorEncoder(w, r, errorx.Internal("MOCK_MARSHAL_ERROR", "marshal error"))
