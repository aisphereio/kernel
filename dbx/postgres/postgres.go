...
// because the target database does not exist (PG SQLSTATE 3D000), open
// calls createDatabase to provision it, then retries the connection
// exactly once. Any other error (auth, network, wrong host) is returned
// without retry.
func open(dsn string, cfg dbx.Config) (*gorm.DB, error) {
	...
	if err != nil {
		...
		// Auto-create the target database and retry.
...
func mapError(err error) error {
	...
	case strings.Contains(lower, "relation") && strings.Contains(lower, "does not exist"),
		...
		return dbx.ErrSchemaNotReady
	case strings.Contains(lower, "sqlstate 3d000"),
