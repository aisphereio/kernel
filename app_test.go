...
func TestApp_Endpoint(t *testing.T) {
	v := []string{"https://kernel.aisphere.io", "localhost"}
...
func TestApp_buildInstance(t *testing.T) {
	want := struct {
		...
		endpoints: []string{"https://kernel.aisphere.io", "localhost"},
	}
	var endpoints []*url.URL
	...
	tests := []fields{
		{
			id:       "1",
			name:     "kernel-v1",
			instance: &registry.ServiceInstance{Endpoints: []string{"https://kernel.aisphere.io", "localhost"}},
			metadata: map[string]string{},
			version:  "v1",
			want: struct {
				id       string
				...
				id: "1", version: "v1", name: "kernel-v1", endpoint: []string{"https://kernel.aisphere.io", "localhost"},
				metadata: map[string]string{},
			},
		},
		{
			...
			metadata: map[string]string{"kernel": "https://github.com/aisphereio/kernel"},
			version:  "v2",
			want: struct {
				id       string
				version  string
				...
				id: "2", version: "v2", name: "kernel-v2", endpoint: []string{"test"},
				metadata: map[string]string{"kernel": "https://github.com/aisphereio/kernel"},
			},
		},
		{
			...
			want: struct {
				...
				id: "3", version: "v3", name: "kernel-v3", endpoint: nil,
				metadata: map[string]string{},
			},
		},
	}
